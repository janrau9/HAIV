...
const NotesNav: React.FC<NotesNavProps> = ({ selectedTab, setSelectedTab }) => {
  return (
    <div className="w-full flex font-bold ">
      <button className={`${selectedTab !== 'case' ? 'bg-notebook-darker' : 'bg-notebook-bg'} px-4 py-2 rounded-t-lg border-1 border-b-0`}  onClick={() => setSelectedTab('case')}>Case</button>
      <button className={`${selectedTab !== 'suspects' ? 'bg-notebook-darker' : 'bg-notebook-bg'} px-4 py-2 rounded-t-lg border-1 border-b-0`} onClick={() => setSelectedTab('suspects')}>Suspects</button>
    </div>
  )
}
